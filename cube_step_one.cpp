...
bool cube_step_one::color_mid_of_edge(char a[3][3], char base){
	return a[0][1] == base || a[1][0] == base || a[1][2] == base || a[2][1] == base;
}
bool cube_step_one::color_floor_mid_of_edge(Cube &A, char base){
	return A.front[0][1] == base || A.left[0][1] == base ||
			A.right[0][1] == base || A.back[0][1] == base;
}
std::string cube_step_one::rotate_cross_down(Cube &A, int base){
	for (int i = 0; i < 4; i++){
		std::string tmp = "";
		if (i == 1) tmp = "d";
		if (i == 2) tmp = "dd";
...
std::string cube_step_one::rotate_cross_floor(Cube &A, int base){
	for (int i = 0; i < 4; i++){
		std::string tmp = "";
		if (i == 1) tmp = "d";
		if (i == 2) tmp = "dd";
...
std::string cube_step_one::rotate_cross_fix(Cube &A, int base){
	if (A.front[2][1] == base){
		A.Turn_f();
		A.Turn_f();
		return "ff";
...
std::string cube_step_one::get_cross(Cube &A){
	std::string step = "";
	char base = A.up[1][1];
...
bool cube_step_one::color_corner_floor(Cube &A, int base){
...
std::string cube_step_one::rotate_corner_floor(Cube &A, int base){
	for (int i = 0; i < 4; i++){
		std::string tmp = "";
		if (i == 1) tmp = "d";
		if (i == 2) tmp = "dd";
...
std::string cube_step_one::rotate_corner_down(Cube &A, int base){
	for (int i = 0; i < 4; i++){
		std::string tmp = "";
		if (i == 1) tmp = "d";
		if (i == 2) tmp = "dd";
...
std::string cube_step_one::rotate_corner_fix(Cube &A, int base){
...
std::string cube_step_one::get_corner(Cube &A){
	std::string step;
	int base = A.up[1][1];
