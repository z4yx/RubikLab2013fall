...
const static char *FINAL_STEP_FORMULA = "rUrururURUrr";

string CStepFinal::corner_place(Cube & cube)
{
	int err = 0;
	string ret;
...
string CStepFinal::dirty_scan(Cube & cube, face_t faces[4])
{
	string ret;
	bool dirty[4] = {0};
	int dirty_cnt = 0;
	...
	if(dirty_cnt == 4){
		...
		cube.ApplyStr(FINAL_STEP_FORMULA);
...
string CStepFinal::run(Cube cubeObj)
{
	string ret;
